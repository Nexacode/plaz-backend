...
class ProjectForecast extends Command
{
     ...
     * @var string
     */
	protected $signature = 'cron:forecast {project}';
    ...
    public function handle()
    {
		...
		//$subprojects = implode(', ', $subprojects);
		...
		$sheet->getStyle($sheet->getCellByColumnAndRow(8, 2)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(8, 2)->getCoordinate())->applyFromArray($styleStatus1);
		$sheet->setCellValue('I2', 'Besprochen');
		$sheet->getStyle($sheet->getCellByColumnAndRow(8, 3)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(8, 3)->getCoordinate())->applyFromArray($styleStatus2);
		$sheet->setCellValue('I3', 'Eingeschaetzt');
		$sheet->getStyle($sheet->getCellByColumnAndRow(8, 4)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(8, 4)->getCoordinate())->applyFromArray($styleStatus3);
		$sheet->setCellValue('I4', 'Freigabe');
		$sheet->getStyle($sheet->getCellByColumnAndRow(8, 5)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(8, 5)->getCoordinate())->applyFromArray($styleStatus4);
		$sheet->setCellValue('I5', 'In Planung');
		$sheet->getStyle($sheet->getCellByColumnAndRow(8, 6)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(8, 6)->getCoordinate())->applyFromArray($styleStatus5);
		$sheet->setCellValue('I6', 'Verzoegert');
		$sheet->getStyle($sheet->getCellByColumnAndRow(8, 7)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(8, 7)->getCoordinate())->applyFromArray($styleStatus6);
		$sheet->setCellValue('I7', 'Im Plan');
		$sheet->getStyle($sheet->getCellByColumnAndRow(8, 8)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(8, 8)->getCoordinate())->applyFromArray($styleStatus7);
		$sheet->setCellValue('I8', 'Programmiert / im Test');
		$sheet->getStyle($sheet->getCellByColumnAndRow(8, 9)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(8, 9)->getCoordinate())->applyFromArray($styleStatus8);
		$sheet->setCellValue('I9', 'Abnahmebereit Testumgebung');
		$sheet->getStyle($sheet->getCellByColumnAndRow(8, 10)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(8, 10)->getCoordinate())->applyFromArray($styleStatus9);
		$sheet->setCellValue('I10', 'Abgenommen Testumgebung');
		$sheet->getStyle($sheet->getCellByColumnAndRow(8, 11)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(8, 11)->getCoordinate())->applyFromArray($styleStatus10);
		$sheet->setCellValue('I11', 'Abnahmebereit');
		$sheet->getStyle($sheet->getCellByColumnAndRow(8, 12)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(8, 12)->getCoordinate())->applyFromArray($styleStatus11);
		$sheet->setCellValue('I12', 'Live / online');
		$sheet->getStyle($sheet->getCellByColumnAndRow(8, 14)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(8, 14)->getCoordinate())->applyFromArray($styleTemporary);
		$sheet->setCellValue('I14', 'Temporaer geplant/ nicht freigegeben. Datum kann sich nach Freigabe aendern');
		...
		foreach($milestones as $milestone){
			...
			if(count($milestone->todo) >= 1){
				...
				$sheet->setCellValue('D' . $z, 'geplant am:');
				$sheet->setCellValue('E' . $z, 'Programmierer');
				$sheet->setCellValue('F' . $z, 'Info');
			}
			...
			foreach($milestone->todo as $todo){
				...
				if($todo->temporary){
					...
					$sheet->getStyle($sheet->getCellByColumnAndRow(1, $z)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(1, $z)->getCoordinate())->applyFromArray($styleTemporary);
					$sheet->getStyle($sheet->getCellByColumnAndRow(3, $z)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(3, $z)->getCoordinate())->applyFromArray($styleTemporary);
					$sheet->getStyle($sheet->getCellByColumnAndRow(4, $z)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(4, $z)->getCoordinate())->applyFromArray($styleTemporary);
					$sheet->getStyle($sheet->getCellByColumnAndRow(5, $z)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(5, $z)->getCoordinate())->applyFromArray($styleTemporary);
					$sheet->getStyle($sheet->getCellByColumnAndRow(6, $z)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(6, $z)->getCoordinate())->applyFromArray($styleTemporary);	
				} else {
					$sheet->setCellValue('A' . $z, $todo->todo);
				}
				...
				if ($todo->todo_status_id == 11){
					$sheet->getStyle($sheet->getCellByColumnAndRow(2, $z)->getCoordinate() .':'. $sheet->getCellByColumnAndRow(2, $z)->getCoordinate())->applyFromArray($styleStatus11);
					$sheet->setCellValue('B' . $z, $todo->statusname->name);
				}				
				...
				if($todo->todo_status_id >= 2 && !empty($todo->deadline)){
					$sheet->setCellValue('D' . $z, Carbon::parse($todo->deadline)->format('d.m.Y'));
				} else {
					$sheet->setCellValue('D' . $z, "");
				}
				
				$sheet->setCellValue('E' . $z, $todo->user_name);
				$sheet->setCellValue('F' . $z, $todo->external_information);
				if($todo->temporary){
					$sheet->setCellValue('G' . $z, "Temporaer geplant / nicht freigegeben");
				}
